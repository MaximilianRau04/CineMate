...
export const applyMediaFilters = (movies, series, filters) => {
  const { searchQuery, selectedGenres, dateRange, contentType, sortOrder } =
    filters;
  ...
  if (selectedGenres.length > 0) {
    tempMovies = tempMovies.filter(
      (movie) =>
        movie.genreArray &&
        movie.genreArray.some((genre) => selectedGenres.includes(genre)),
    );

    tempSeries = tempSeries.filter(
      (series) =>
        series.genreArray &&
        series.genreArray.some((genre) => selectedGenres.includes(genre)),
    );
  }
  ...
  if (dateRange.start) {
    ...
    tempMovies = tempMovies.filter(
      (movie) => movie.releaseDate && new Date(movie.releaseDate) >= startDate,
    );
  ...
  if (dateRange.end) {
    ...
    tempMovies = tempMovies.filter(
      (movie) => movie.releaseDate && new Date(movie.releaseDate) <= endDate,
    );

    tempSeries = tempSeries.filter(
      (series) => series.releaseDate && new Date(series.releaseDate) <= endDate,
    );
  }
