...
public class ReviewService {
  ...
  public ReviewResponseDTO createReview(ReviewRequestDTO reviewRequestDTO) {
    ...
    eventPublisher.publishEvent(
        new com.cinemate.notification.events.UserActivityEvent(
            this,
            savedReview.getUserId(),
            com.cinemate.notification.events.UserActivityEvent.ActivityType.REVIEW_CREATED,
            savedReview.getItemId()));
  ...
  public Optional<ReviewResponseDTO> getReviewById(String id) {
    return reviewRepository.findById(id).map(ReviewResponseDTO::new);
  }
  ...
  private String getItemTitle(String itemType, String itemId) {
    if (TYPE_MOVIE.equals(itemType)) {
      return movieRepository.findById(itemId).map(Movie::getTitle).orElse("Unbekannter Film");
    } else if (TYPE_SERIES.equals(itemType)) {
      return seriesRepository.findById(itemId).map(Series::getTitle).orElse("Unbekannte Serie");
    }
