...
public class CustomListService {
  ...
  @Autowired private CustomListRepository customListRepository;

  @Autowired private ListCommentRepository commentRepository;
   ...
   * @param requestDTO The data transfer object containing the details for the custom list,
   *     including title, description, privacy settings, tags, and cover image URL.
   * @return A ResponseEntity containing a CustomListResponseDTO with details of the created custom
   *     list, or a bad request response if the user does not exist.
   */
  public ResponseEntity<CustomListResponseDTO> createCustomList(
      String userId, CustomListRequestDTO requestDTO) {
    ...
    CustomList customList =
        new CustomList(
            requestDTO.getTitle(), requestDTO.getDescription(), user, requestDTO.isPublic());
  ...
  /**
   * Retrieves a custom list by its unique identifier and checks access permissions for the current
   * user.
   *
   ...
   * @return a {@code ResponseEntity} containing a {@code CustomListResponseDTO} if the list is
   *     found and accessible;
   */
  public ResponseEntity<CustomListResponseDTO> getCustomListById(
      String listId, String currentUserId) {
  ...
  /**
   * Updates an existing custom list with new data provided in the request. Validates the list ID,
   * user access, and updates the information based on the request details.
   *
   ...
   * @return A ResponseEntity containing the updated custom list data if successful. Returns a 404
   *     status if the list is not found or a 403 status if the user is not authorized.
   */
  public ResponseEntity<CustomListResponseDTO> updateCustomList(
      String listId, String userId, CustomListRequestDTO requestDTO) {
    Optional<CustomList> listOptional = customListRepository.findById(listId);
    if (listOptional.isEmpty()) {
      return ResponseEntity.notFound().build();
    }
  ...
  /**
   * Deletes a custom list identified by the given list ID if it exists and the user has permission
   * to do so. The method first checks whether the list exists. If the list does not exist, a 404
  ...
  public ResponseEntity<List<CustomListResponseDTO>> getUserLists(
    ...
    List<CustomListResponseDTO> responseDTOs =
        lists.stream().map(CustomListResponseDTO::new).collect(Collectors.toList());
   ...
   * @param sortBy the sorting criteria; "popular" for sorting by likes and updated date in
   *     descending order, or "recent" (default) for sorting by updated date in descending order
  ...
  public ResponseEntity<Page<CustomListResponseDTO>> searchLists(String query, int page, int size) {
    Pageable pageable = PageRequest.of(page, size);
    Page<CustomList> lists =
        customListRepository.findByTitleContainingIgnoreCaseAndIsPublicTrue(query, pageable);
    Page<CustomListResponseDTO> responseDTOs = lists.map(CustomListResponseDTO::new);
    return ResponseEntity.ok(responseDTOs);
  }
   ...
   * @return a {@code ResponseEntity<CustomListResponseDTO>} containing the updated custom list if
   *     the process is successful, or an error response entity if the operation fails
   */
  public ResponseEntity<CustomListResponseDTO> addMovieToList(
      String listId, String movieId, String userId) {
    Optional<CustomList> listOptional = customListRepository.findById(listId);
  ...
  /**
   * Removes a movie from a custom list based on the provided list ID and movie ID. Validates if the
   * list and movie exist, and verifies the user's authorization to modify the list before
   ...
   * @return a ResponseEntity containing a {@code CustomListResponseDTO} if the operation is
   *     successful,
   */
  ...
  /**
   * Adds a series to a custom list if the list and series exist, and the user is authorized to
   * modify the list. Returns the updated custom list.
   *
   * @param listId the unique identifier of the custom list
   ...
   * @return a ResponseEntity containing the updated CustomListResponseDTO if successful, or an
   *     appropriate HTTP status if the operation fails
   */
  public ResponseEntity<CustomListResponseDTO> addSeriesToList(
      String listId, String seriesId, String userId) {
    Optional<CustomList> listOptional = customListRepository.findById(listId);
  ...
  /**
   * Toggles the like status of a custom list for a specified user. If the custom list is private,
   * only the creator of the list is permitted to like or unlike the list.
   *
  ...
  /**
   * Adds a comment to a custom list. Ensures the user and the list exist, and validates if the user
   * has permission to comment on the specified list.
   *
   ...
   * @return a ResponseEntity containing a ListCommentResponseDTO of the saved comment if
   *     successful, or an appropriate response status if the operation fails
   */
  public ResponseEntity<ListCommentResponseDTO> addComment(
      String listId, String userId, String content) {
    Optional<CustomList> listOptional = customListRepository.findById(listId);
    Optional<User> userOptional = userRepository.findById(userId);
  ...
  /**
   * Retrieves the comments for a specified list. This method checks if the list exists and verifies
   * if the current user has access to the comments.
   *
   ...
   * @return a ResponseEntity containing a list of ListCommentResponseDTO objects representing the
   *     comments if successful, or an appropriate error status
   */
  public ResponseEntity<List<ListCommentResponseDTO>> getComments(
    ...
    List<ListComment> comments = commentRepository.findByCustomListOrderByCreatedAtDesc(customList);
    List<ListCommentResponseDTO> responseDTOs =
        comments.stream().map(ListCommentResponseDTO::new).collect(Collectors.toList());
