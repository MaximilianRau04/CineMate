...
public class CustomListController {
  ...
  @Autowired private CustomListService customListService;
  ...
  public ResponseEntity<CustomListResponseDTO> createCustomList(
      @AuthenticationPrincipal User user, @Valid @RequestBody CustomListRequestDTO requestDTO) {
    return customListService.createCustomList(user.getId(), requestDTO);
  }
   ...
   * @param user the currently authenticated user
   * @return a {@code ResponseEntity} containing the custom list data, or an appropriate HTTP status
   *     if not found
   */
  @GetMapping("/{listId}")
  public ResponseEntity<CustomListResponseDTO> getCustomListById(
      @PathVariable String listId, @AuthenticationPrincipal User user) {
  ...
  public ResponseEntity<CustomListResponseDTO> updateCustomList(
    ...
    return customListService.updateCustomList(listId, user.getId(), requestDTO);
  }
  ...
  public ResponseEntity<Void> deleteCustomList(
      @PathVariable String listId, @AuthenticationPrincipal User user) {
    return customListService.deleteCustomList(listId, user.getId());
  }

  /**
   * Retrieves the lists for a specified user. If the currently authenticated user is provided, the
   * response may include additional data based on their permissions or related context.
   *
  ...
  public ResponseEntity<List<CustomListResponseDTO>> getUserLists(
      @PathVariable String userId, @AuthenticationPrincipal User user) {
    String currentUserId = user != null ? user.getId() : null;
  ...
  public ResponseEntity<List<CustomListResponseDTO>> getMyLists(
      @AuthenticationPrincipal User user) {
    return customListService.getUserLists(user.getId(), user.getId());
  }
   ...
   * @return a {@code ResponseEntity} containing a {@code Page} of {@code CustomListResponseDTO}
   *     objects
   */
  @GetMapping("/public")
  ...
  /**
   * Searches for custom lists based on a query string, and retrieves a paginated response with the
   * search results.
   *
  ...
  public ResponseEntity<CustomListResponseDTO> addMovieToList(
      ...
      @AuthenticationPrincipal User user) {
    return customListService.addMovieToList(listId, movieId, user.getId());
  }
   ...
   * @return a ResponseEntity containing a CustomListResponseDTO reflecting the updated state of the
   *     custom list
   */
  ...
  public ResponseEntity<CustomListResponseDTO> removeMovieFromList(
      ...
      @PathVariable String movieId,
      @AuthenticationPrincipal User user) {
    return customListService.removeMovieFromList(listId, movieId, user.getId());
  }
  ...
  public ResponseEntity<CustomListResponseDTO> addSeriesToList(
      ...
      @AuthenticationPrincipal User user) {
    return customListService.addSeriesToList(listId, seriesId, user.getId());
  }
  ...
  public ResponseEntity<CustomListResponseDTO> removeSeriesFromList(
      ...
      @PathVariable String seriesId,
      @AuthenticationPrincipal User user) {
    return customListService.removeSeriesFromList(listId, seriesId, user.getId());
  }

  /**
   * Toggles the 'like' status for a specified list. If the user has already liked the list, this
   * action will remove the like. Otherwise, it will add a like to the list.
   *
   ...
   * @return a ResponseEntity containing the updated list data encapsulated in a
   *     CustomListResponseDTO
   */
  @PostMapping("/{listId}/like")
  public ResponseEntity<CustomListResponseDTO> toggleLike(
      @PathVariable String listId, @AuthenticationPrincipal User user) {
    return customListService.toggleLike(listId, user.getId());
  }
   ...
   * @param userDetails the details of the authenticated user making the request
   * @param request a map containing the comment data, where the key is "content" and the value is
   *     the comment text
  ...
  public ResponseEntity<ListCommentResponseDTO> addComment(
    ...
    return customListService.addComment(listId, user.getId(), content);
  }
   ...
   * @return a ResponseEntity containing a list of ListCommentResponseDTO objects that represent the
   *     comments
   */
  ...
  /**
   * Deletes a comment identified by its ID. The comment is removed if it belongs to the
   * authenticated user.
   *
  ...
  public ResponseEntity<Void> deleteComment(
      @PathVariable String commentId, @AuthenticationPrincipal User user) {
    return customListService.deleteComment(commentId, user.getId());
  }
}
