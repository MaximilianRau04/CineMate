...
public class RecommendationService {
  ...
  /**
   * Generates personalized recommendations for a user based on Content-Based Filtering
   *
   * @param userId
  ...
  public List<RecommendationResponseDTO> getRecommendationsForUser(String userId) {
    ...
    for (Movie movie : candidateMovies) {
      double score =
          calculateContentScore(movie, preferredGenres, preferredActors, preferredDirectors);
    ...
    for (Series series : candidateSeries) {
      double score =
          calculateContentScore(series, preferredGenres, preferredActors, preferredDirectors);
      if (score > 0.3) { // Minimum threshold
        recommendations.add(
            new RecommendationResponseDTO(
                series.getId(),
                ...
                "series",
                score,
                generateReasonForSeries(
                    series, preferredGenres, preferredActors, preferredDirectors),
                series.getPosterUrl()));
      }
    }
  ...
  private Set<String> extractUserPreferredGenres(User user) {
    ...
    // From favorites
    user.getMovieFavorites()
        .forEach(movie -> genres.addAll(Arrays.asList(movie.getGenre().split(",\\s*"))));
    user.getSeriesFavorites()
        .forEach(series -> genres.addAll(Arrays.asList(series.getGenre().split(",\\s*"))));

    // From watched content (with lower weight)
    user.getMoviesWatched()
        .forEach(movie -> genres.addAll(Arrays.asList(movie.getGenre().split(",\\s*"))));
    user.getSeriesWatched()
        .forEach(series -> genres.addAll(Arrays.asList(series.getGenre().split(",\\s*"))));
  ...
  /**
   * Core scoring logic shared by movie and series overloads. Genre: 40%, Directors: 30%, Actors:
   * 20%, Rating: 10%
   */
  private double calculateContentScore(
      String genre,
    ...
    if (actors != null) {
      long actorMatches =
          actors.stream().filter(actor -> preferredActors.contains(actor.getId())).count();
  ...
  /**
   * Retrieves a set of unique movie IDs associated with the given user. This includes movies from
   * the user's favorite list, watched list, and watchlist.
   *
  ...
  /**
   * Retrieves a set of unique series IDs associated with a user by combining their favorite series,
   * watched series, and series in the watchlist.
   *
   * @param user the user whose series IDs are to be retrieved
   * @return a set containing unique series IDs from the user's favorites, watched list, and
   *     watchlist
   */
  ...
  /**
   * Generates a reason describing why a specific movie might align with a user's preferences, based
   * on genres, actors, and directors.
   *
   * @param movie The movie for which the reason is being generated. Contains details such as genre,
   *     actors, and directors.
   * @param preferredGenres A set of the user's preferred genres to be matched with the movie's
   *     genres.
   * @param preferredActors A set of the user's preferred actors to be matched with the movie's
   *     actors.
   * @param preferredDirectors A set of the user's preferred directors to be matched with the
   *     movie's directors.
   * @return A string describing the reason why the movie corresponds to the user's preferences, or
   *     a default message if no matches are found.
   */
  private String generateReasonForMovie(
      Movie movie,
      Set<String> preferredGenres,
      Set<String> preferredActors,
      Set<String> preferredDirectors) {
    ...
    if (movie.getGenre() != null) {
      ...
      List<String> matchingGenres =
          Arrays.stream(movieGenres).filter(preferredGenres::contains).collect(Collectors.toList());
   ...
   * @return a string explaining the reasons for recommending the series based on matches with the
   *     user's preferences
   */
  private String generateReasonForSeries(
      Series series,
      Set<String> preferredGenres,
      Set<String> preferredActors,
      Set<String> preferredDirectors) {
  ...
  /**
   * Fetches a list of trending recommendations, including movies and series, based on a weighted
   * score derived from their ratings and number of reviews. The returned list combines top-rated
   * movies and series, converts them into
   *
   * @return A list of {@link RecommendationResponseDTO} objects representing trending movies and
   *     series.
   */
  ...
  /**
   * Retrieves a list of recommendations based on the specified genre. The recommendations include
   * both movies and series, ranked by their ratings.
   *
   * @param genre the genre used to filter the movies and series recommendations
   * @return a list of {@link RecommendationResponseDTO} containing the recommended movies and
   *     series for the specified genre
   */
  public List<RecommendationResponseDTO> getRecommendationsByGenre(String genre) {
    ...
    // Series of the genre
    List<Series> genreSeries =
        seriesRepository.findAll().stream()
            .filter(
                series ->
                    series.getGenre() != null
  ...
  /**
   * Generates a list of recommendations based on a collaborative filtering approach. Finds users
   * with similar preferences to the provided user and recommends their favorite movies and series
   * that the current user has not yet engaged with.
   *
   * @param userId the unique identifier of the user for whom the recommendations are to be
   *     generated
   * @return a list of {@code RecommendationResponseDTO} objects containing recommended movies and
   *     series along with their details and scores; returns an empty list if no similar users or
   *     recommendations are found
   */
  public List<RecommendationResponseDTO> getCollaborativeRecommendations(String userId) {
    // Here a Collaborative Filtering algorithm could be implemented that finds users with similar
    // preferences and recommends their favorites
          ...
          .forEach(
              movie -> {
                if (!currentUserMovieIds.contains(movie.getId())) {
                  movieRecommendations.put(
                      movie.getId(), movieRecommendations.getOrDefault(movie.getId(), 0) + 1);
                }
              });
          ...
          .forEach(
              series -> {
                if (!currentUserSeriesIds.contains(series.getId())) {
                  seriesRecommendations.put(
                      series.getId(), seriesRecommendations.getOrDefault(series.getId(), 0) + 1);
                }
              });
    }
  ...
  /**
   * Finds and returns a list of users similar to the given user based on shared interests such as
   * movies and series.
   *
   ...
   * @return a list of up to 5 users who are most similar to the current user, sorted in descending
   *     order of similarity
   */
  ...
  /**
   * Calculates the Jaccard similarity between two sets of strings. The Jaccard similarity is
   * defined as the size of the intersection divided by the size of the union of the two sets.
   *
  ...
  /**
   * Generates a hybrid recommendation list by combining content-based and collaborative filtering
   * approaches with specific weightings. Content-based recommendations are given a 70% weighting,
   * while collaborative recommendations are weighted at 30%.
   *
   * @param userId The unique identifier of the user for whom the hybrid recommendations are being
   *     generated.
   * @return A list of RecommendationResponseDTO objects representing the hybrid recommendations,
   *     sorted by their scores in descending order and limited to 15 items.
   */
  ...
  /**
   * Provides smart content recommendations for a user based on the time of day. The method
   * leverages different recommendation strategies to suggest content aligning with the user's
   * potential mood or preference for a specific time window.
   *
   * @param userId a unique identifier for the user requesting recommendations
   * @return a list of RecommendationResponseDTO objects containing the smart recommendations for
   *     the user
   */
  ...
  /**
   * Provides a list of recommended movies and series based on the user's mood and preferences. The
   * recommendations are filtered by mood-specific genres and exclude content the user has already
   * watched.
   *
   * @param userId The unique identifier of the user for whom recommendations are generated.
   * @param mood The mood of the user, which determines the genre and tone of content to be
   *     recommended. Supported moods include "light", "evening", and "night".
   * @return A list of recommendations containing movies and series tailored to the user's mood, or
   *     an empty list if the user is not found or there are no recommendations available.
   */
                ...
                series ->
                    series.getGenre() != null
                        && moodGenres.stream()
                            .anyMatch(
                                genre ->
                                    Arrays.asList(series.getGenre().split(",\\s*"))
                                        .contains(genre)))
            .sorted((a, b) -> Double.compare(b.getRating(), a.getRating()))
            .limit(10)
            .collect(Collectors.toList());
