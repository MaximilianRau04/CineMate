...
public class SeriesService {
  ...
  public ResponseEntity<List<SeriesResponseDTO>> getAllSeries() {
    ...
    List<SeriesResponseDTO> seriesDTOs =
        seriesList.stream().map(SeriesResponseDTO::new).collect(Collectors.toList());
  ...
  public Optional<SeriesResponseDTO> getSeriesById(String id) {
    return seriesRepository.findById(id).map(SeriesResponseDTO::new);
  }
  ...
  public ResponseEntity<SeriesResponseDTO> updateSeries(String id, SeriesRequestDTO seriesDTO) {
    ...
    if (seriesDTO.getStatus() != null
        && oldStatus != null
        && !oldStatus.equals(seriesDTO.getStatus())) {
  ...
  public ResponseEntity<List<Season>> getSeasons(String seriesId) {
    ...
    if (seasons == null) {
      ...
      List<Season> validSeasons =
          seasons.stream().filter(Objects::nonNull).collect(Collectors.toList());
  ...
  public ResponseEntity<Season> getSeason(String seriesId, int seasonNumber) {
    return findAndProcessEntity(
        seriesId, series -> findSeasonByNumber(series, seasonNumber).orElse(null));
  }
  ...
  public ResponseEntity<Season> updateSeason(
      String seriesId, int seasonNumber, Season updatedSeason) {
    Optional<Series> optionalSeries = seriesRepository.findById(seriesId);
    if (optionalSeries.isEmpty()) {
      return ResponseEntity.notFound().build();
    }
    ...
    if (updatedSeason.getSeasonNumber() != -1)
  ...
  public ResponseEntity<Episode> addEpisode(String seriesId, int seasonNumber, Episode newEpisode) {
    if (newEpisode.getTitle() == null
        || newEpisode.getReleaseDate() == null
        || newEpisode.getEpisodeNumber() <= 0) {
      return ResponseEntity.badRequest().build();
    }
    ...
    Optional<Episode> addedEpisode =
        findEpisodeByNumber(savedSeason.get(), newEpisode.getEpisodeNumber());

    if (addedEpisode.isPresent()) {
      eventPublisher.publishEvent(
          new SeriesUpdatedEvent(this, savedSeries, savedSeason.get(), addedEpisode.get()));
    }
  ...
  public ResponseEntity<Episode> updateEpisode(
      String seriesId, int seasonNumber, int episodeNumber, Episode updatedEpisode) {
    Optional<Series> optionalSeries = seriesRepository.findById(seriesId);
    if (optionalSeries.isEmpty()) {
      return ResponseEntity.notFound().build();
    }
  ...
  private void updateEpisodeFields(Episode existingEpisode, Episode updatedEpisode) {
    ...
    if (updatedEpisode.getDescription() != null)
      existingEpisode.setDescription(updatedEpisode.getDescription());
    if (updatedEpisode.getDuration() != null)
      existingEpisode.setDuration(updatedEpisode.getDuration());
    if (updatedEpisode.getReleaseDate() != null)
      existingEpisode.setReleaseDate(updatedEpisode.getReleaseDate());
    if (updatedEpisode.getPosterUrl() != null)
      existingEpisode.setPosterUrl(updatedEpisode.getPosterUrl());
  }
   ...
   * @param processor
   */
  private <T> ResponseEntity<T> findAndProcessEntity(
      String seriesId, Function<Series, T> processor) {
    Optional<Series> optionalSeries = seriesRepository.findById(seriesId);
  ...
  public ResponseEntity<DirectorResponseDTO> addDirectorToSeries(
      String seriesId, String directorId) {
    Optional<Series> optionalSeries = seriesRepository.findById(seriesId);
