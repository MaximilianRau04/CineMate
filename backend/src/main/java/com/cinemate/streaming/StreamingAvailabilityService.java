...
public class StreamingAvailabilityService {
  ...
  public ResponseEntity<List<StreamingAvailabilityResponseDTO>> getAvailabilityForMedia(
    ...
    List<StreamingAvailability> availabilities =
        availabilityRepository.findByMediaIdAndMediaType(mediaId, type);

    List<StreamingAvailabilityResponseDTO> response =
        availabilities.stream()
            .filter(
                availability -> availability.getProvider() != null) // Filtere null-Provider heraus
  ...
  public ResponseEntity<List<StreamingAvailabilityResponseDTO>> getAvailabilityForMediaAndRegion(
    ...
    MediaType type = MediaType.valueOf(mediaType.toUpperCase());
    List<StreamingAvailability> availabilities =
        availabilityRepository.findByMediaIdAndMediaTypeAndRegion(mediaId, type, region);
  ...
  public ResponseEntity<StreamingAvailabilityResponseDTO> addAvailability(
    ...
    StreamingAvailability availability =
        new StreamingAvailability(mediaId, type, provider, avType, region);
  ...
  public ResponseEntity<StreamingAvailabilityResponseDTO> updateAvailability(
    ...
    StreamingAvailability availability =
        availabilityRepository.findById(availabilityId).orElse(null);
