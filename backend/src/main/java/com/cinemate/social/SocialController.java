...
public class SocialController {
  ...
  public ResponseEntity<?> sendFriendRequest(
      @PathVariable String targetUserId, Authentication authentication) {
   ...
   * @param authentication the authentication object containing details of the currently
   *     authenticated user
   * @return a ResponseEntity containing the result of the operation, including success or error
   *     messages
   */
  @PostMapping("/friends/accept/{friendshipId}")
  public ResponseEntity<?> acceptFriendRequest(
      @PathVariable String friendshipId, Authentication authentication) {
    String currentUserId = getCurrentUserId(authentication);
    if (currentUserId == null) {
      return ResponseEntity.status(401).body("Not authenticated");
    }
   ...
   * @param authentication the authentication object containing details of the currently
   *     authenticated user
   * @return a ResponseEntity containing the result of the operation, such as success or error
   *     messages
   */
   ...
   * @return a ResponseEntity containing a list of UserResponseDTO objects that represent the
   *     friends of the user
   */
   ...
   * @param authentication the authentication object containing details of the currently
   *     authenticated user
   * @return a ResponseEntity containing a list of FriendRequestDTO objects representing the pending
   *     friend requests
   */
  ...
  public ResponseEntity<?> removeFriend(
      @PathVariable String friendUserId, Authentication authentication) {
    String currentUserId = getCurrentUserId(authentication);
    if (currentUserId == null) {
      return ResponseEntity.status(401).body("Not authenticated");
    }
   ...
   *
   * @param limit the maximum number of users to include in the leaderboard; defaults to 10 if not
   *     specified
   * @return a ResponseEntity containing a list of UserPointsDTO objects representing the users and
   *     their points
   */
  ...
  public ResponseEntity<?> testAwardPoints(
      @PathVariable String userId,
      @RequestParam String type,
      @RequestParam(defaultValue = "0") int points) {
    try {
      com.cinemate.social.points.PointsType pointsType =
          com.cinemate.social.points.PointsType.valueOf(type.toUpperCase());
