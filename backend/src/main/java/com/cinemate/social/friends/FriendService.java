...
public class FriendService {
  ...
  public ResponseEntity<?> sendFriendRequest(String currentUserId, String targetUserId) {
    try {
      ...
      Optional<Friend> existingFriendship =
          friendRepository.findFriendshipBetweenUsers(currentUser, targetUser);
  ...
  public ResponseEntity<?> acceptFriendRequest(String currentUserId, String friendshipId) {
    try {
      ...
      notificationService.sendNotification(
          ...
          String.format(
              "%s hat deine Freundschaftsanfrage akzeptiert!",
      ...
      eventPublisher.publishEvent(
          new AchievementCheckEvent(this, currentUserId, "friend_accepted"));
      eventPublisher.publishEvent(
          new AchievementCheckEvent(this, friendship.getRequester().getId(), "friend_accepted"));
   ...
   * @return a ResponseEntity containing a list of UserResponseDTO objects representing the user's
   *     friends,
   */
  ...
  public ResponseEntity<List<FriendRequestDTO>> getPendingRequests(String userId) {
    try {
      ...
      List<Friend> pendingRequests =
          friendRepository.findByRecipientAndStatus(user, FriendshipStatus.PENDING);

      List<FriendRequestDTO> requests =
          pendingRequests.stream().map(FriendRequestDTO::new).collect(Collectors.toList());
  ...
  /**
   * Removes an existing friendship between two users. If the friendship does not exist or if either
   * user is not found, an appropriate response is returned.
   *
  ...
  public ResponseEntity<?> removeFriend(String currentUserId, String friendUserId) {
    try {
      ...
      Optional<Friend> friendshipOpt =
          friendRepository.findAcceptedFriendshipBetweenUsers(currentUser, friendUser);
      if (friendshipOpt.isEmpty()) {
