...
public class ForumDTOConverter {
  ...
  public List<ForumPostDTO> convertToDTO(List<ForumPost> posts) {
    return posts.stream().map(ForumPostDTO::new).collect(Collectors.toList());
  }

  public Page<ForumPostDTO> convertToDTO(Page<ForumPost> posts) {
    List<ForumPostDTO> dtoList =
        posts.getContent().stream().map(ForumPostDTO::new).collect(Collectors.toList());
