...
/**
 * REST Controller for managing forum operations. Provides endpoints for creating, reading,
 * updating, and deleting forum posts and replies. Includes functionality for subscription
 * management, search, and administrative operations.
 *
...
public class ForumController {
   ...
   *
   * @return the authenticated user's ID, or null if the request is unauthenticated
   */
  ...
  /**
   * Handles the creation of a new forum post. The authenticated user is associated with the post as
   * the creator. Responds with the created post along with the status.
   *
   ...
   */
  @PostMapping("/posts")
  public ResponseEntity<ForumPostDTO> createPost(@Valid @RequestBody ForumPost post) {
  ...
  /**
   * Retrieves a paginated list of forum posts, optionally filtered by category, media type, or
   * sorted by criteria.
   *
   ...
   *
   * @return a ResponseEntity containing a list of pinned forum post DTOs as the body of the
   *     response
   */
   ...
   * @return a ResponseEntity containing a Page of ForumPostDTO objects authored by the specified
   *     user
   */
   ...
   * @return a ResponseEntity containing a Page of ForumPostDTO objects associated with the
   *     specified series
   */
   ...
   * @return a ResponseEntity containing a paginated list of forum post DTOs the user has
   *     participated in
   */
  ...
  /**
   * Updates an existing forum post by its identifier. The method ensures that only an authenticated
   * user with the necessary permissions can update the post.
   *
  ...
  public ResponseEntity<ForumPost> updatePost(
      @PathVariable String id, @Valid @RequestBody ForumPost post) {
    String userId = getAuthenticatedUserId();
  ...
  /**
   * Toggles the like status of a forum post for the authenticated user. If the user already likes
   * the post, the like is removed. Otherwise, the user likes the post.
   *
  ...
  public ResponseEntity<ForumReply> createReply(
      @PathVariable String postId, @RequestBody ForumReply reply) {
    String userId = getAuthenticatedUserId();
   ...
   * @return a ResponseEntity containing a Page of ForumReply objects, which represents the
   *     paginated list of replies for the given post
   */
  ...
  /**
   * Updates an existing forum reply with the specified id. The reply is updated if the
   * authenticated user has permission to modify it.
   *
  ...
  public ResponseEntity<ForumReply> updateReply(
      @PathVariable String id, @RequestBody ForumReply reply) {
    String userId = getAuthenticatedUserId();
   ...
   * @return a ResponseEntity with no content if the deletion is successful, or a FORBIDDEN status
  ...
  /**
   * Toggles the like status for a forum reply by the currently authenticated user. If the user has
   * already liked the reply, the like is removed. If the user has not liked the reply, a like is
   * added.
   *
  ...
  /**
   * Handles the unsubscription of a user from a specific post. This method allows authenticated
   * users to unsubscribe from receiving notifications or updates about a specified post.
   *
   ...
   * @return a ResponseEntity containing a SubscriptionStatus object, which includes the
   *     subscription status of the user and the total number of subscribers for the post
   */
  ...
  public ResponseEntity<SubscriptionStatus> getSubscriptionStatus(@PathVariable String postId) {
    ...
    boolean isSubscribed = userId != null && forumService.isUserSubscribedToPost(userId, postId);
   ...
   *
   * @return ResponseEntity containing a list of ForumSubscription objects representing the user's
   *     subscriptions on success, or ResponseEntity with a BAD_REQUEST status if an error occurs.
   */
   ...
   * @return a ResponseEntity containing ForumUserStats with the post count and reply count for the
   *     specified user
   */
  ...
  /**
   * Pins or unpins a forum post based on the provided pinned status. This endpoint is intended for
   * administrative use.
   *
  ...
  /**
   * Admin endpoint to delete posts without authorization checks. Useful for cleaning up posts from
   * deleted users.
   *
