...
/**
 * Service class for managing forum operations. Provides business logic for forum posts, replies,
 * subscriptions, and related functionality. Handles data persistence, event publishing, and
...
public class ForumService {
  ...
  /**
   * Creates a new forum post with the specified user as the author. Automatically subscribes the
   * author to their own post and publishes an event for notifications.
   *
  ...
  public Page<ForumPost> getPostsByCategory(ForumCategory category, Pageable pageable) {
    return forumPostRepository.findByCategoryAndIsDeletedFalseOrderByCreatedAtDesc(
        category, pageable);
  }
  ...
  public Page<ForumPost> getPostsByAuthor(String authorId, Pageable pageable) {
    return forumPostRepository.findByAuthorIdAndIsDeletedFalseOrderByCreatedAtDesc(
        authorId, pageable);
  }
  ...
  public Page<ForumPost> getPostsByMovieId(String movieId, Pageable pageable) {
    return forumPostRepository.findByMovieIdAndIsDeletedFalseOrderByCreatedAtDesc(
        movieId, pageable);
  }
  ...
  public Page<ForumPost> getPostsBySeriesId(String seriesId, Pageable pageable) {
    return forumPostRepository.findBySeriesIdAndIsDeletedFalseOrderByCreatedAtDesc(
        seriesId, pageable);
  }
  ...
  /**
   * Retrieves forum posts where the specified user has participated (either as author or replied
   * to).
   *
  ...
  /**
   * Updates a forum post. Only the author can edit their post (not admins).
   *
  ...
  /**
   * Marks a forum post as deleted (soft delete). Only the author of the post can delete it.
   *
  ...
  /**
   * Admin method to delete posts without authorization checks. Useful for cleaning up posts from
   * deleted users.
   *
   * @param postId the ID of the post to delete
   * @throws ResourceNotFoundException if the post is not found
  ...
  /**
   * Creates a new reply to a forum post. Updates the parent post's reply count and last modified
   * date. Publishes an event for notification purposes.
   *
  ...
  public Page<ForumReply> getRepliesForPost(String postId, Pageable pageable) {
    return forumReplyRepository.findByParentPostIdAndIsDeletedFalseOrderByCreatedAtAsc(
        postId, pageable);
  }
  ...
  public Page<ForumReply> getRepliesByAuthor(String authorId, Pageable pageable) {
    return forumReplyRepository.findByAuthorIdAndIsDeletedFalseOrderByCreatedAtDesc(
        authorId, pageable);
  }

  /**
   * Updates a forum reply. Only the author can edit their reply (not admins).
   *
  ...
  /**
   * Marks a forum reply as deleted (soft delete). Only the author of the reply or an admin can
   * delete it.
   *
  ...
  /**
   * Pins or unpins a forum post. Pinned posts are displayed at the top of the forum.
   *
  ...
  /**
   * Locks or unlocks a forum post. Locked posts cannot receive new replies.
   *
  ...
  /**
   * Subscribes a user to a forum post. If the user is already subscribed, returns the existing
   * subscription.
   *
  ...
  /**
   * Unsubscribes a user from a forum post. Marks the subscription as inactive instead of deleting
   * it.
   *
  ...
  public boolean isUserSubscribedToPost(String userId, String postId) {
    return forumSubscriptionRepository
        .findByUserIdAndPostIdAndIsActiveTrue(userId, postId)
        .isPresent();
  }
  ...
  public Page<ForumPost> getPostsByMediaType(String mediaType, Pageable pageable) {
    switch (mediaType.toLowerCase()) {
      case "movie":
        return forumPostRepository.findByMovieIdIsNotNullAndIsDeletedFalseOrderByCreatedAtDesc(
            pageable);
      case "series":
        return forumPostRepository.findBySeriesIdIsNotNullAndIsDeletedFalseOrderByCreatedAtDesc(
            pageable);
      case "none":
        return forumPostRepository
            .findByMovieIdIsNullAndSeriesIdIsNullAndIsDeletedFalseOrderByCreatedAtDesc(pageable);
      default:
   ...
   */
  public Page<ForumPost> getPostsByCategoryAndMediaType(
      ForumCategory category, String mediaType, Pageable pageable) {
    switch (mediaType.toLowerCase()) {
      case "movie":
        return forumPostRepository
            .findByCategoryAndMovieIdIsNotNullAndIsDeletedFalseOrderByCreatedAtDesc(
                category, pageable);
      case "series":
        return forumPostRepository
            .findByCategoryAndSeriesIdIsNotNullAndIsDeletedFalseOrderByCreatedAtDesc(
                category, pageable);
      case "none":
        return forumPostRepository
            .findByCategoryAndMovieIdIsNullAndSeriesIdIsNullAndIsDeletedFalseOrderByCreatedAtDesc(
                category, pageable);
      default:
