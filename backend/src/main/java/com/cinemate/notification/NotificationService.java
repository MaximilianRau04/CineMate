...
public class NotificationService {
   ...
   * @return Notification
   */
  public Notification createNotificationWithMetadata(
      String userId,
  ...
  public void sendNotification(String notificationId) {
    ...
    boolean shouldSendEmail =
        user.isEmailNotificationsEnabled()
            && isNotificationTypeEnabled(user, notification.getType(), true);
    boolean shouldSendWeb =
        user.isWebNotificationsEnabled()
            && isNotificationTypeEnabled(user, notification.getType(), false);
  ...
  private void sendEmailForNotificationType(Notification notification, User user) {
    ...
    emailService.sendTemplatedNotificationEmail(
        user.getEmail(), notification.getTitle(), notification.getMessage(), actionUrl, actionText);
  }
  ...
  public void sendNotificationToAdmins(NotificationType type, String title, String message) {
    ...
    for (User admin : adminUsers) {
      try {
        ...
        log.error(
            "Fehler beim Senden der Benachrichtigung an Admin "
                + admin.getUsername()
                + ": "
                + e.getMessage());
      }
  ...
  public void sendAdminNotification(
      NotificationType type, String title, String message, String targetUserId) {
    if (targetUserId != null) {
      try {
        sendNotification(targetUserId, type, title, message);
      } catch (Exception e) {
        log.error(
            "Fehler beim Senden der Admin-Benachrichtigung an User "
                + targetUserId
                + ": "
                + e.getMessage());
      }
