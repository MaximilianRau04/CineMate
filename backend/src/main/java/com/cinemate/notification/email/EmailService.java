...
public class EmailService {
  ...
  public void sendHtmlNotificationEmail(String toEmail, String subject, String htmlContent) {
    if (!mailEnabled || mailSender == null) {
      log.info(
          "Email service not configured. Would send HTML email to " + toEmail + ": " + subject);
      return;
    }
   ...
   */
  @Async
  public void sendTemplatedNotificationEmail(
      String toEmail, String title, String message, String actionUrl, String actionText) {
    String htmlContent =
        emailTemplateService.createNotificationEmailTemplate(title, message, actionUrl, actionText);
    sendHtmlNotificationEmail(toEmail, title, htmlContent);
  }

  /**
   * Sends a weekly summary email to a user asynchronously. The email content is generated using a
   * template that includes details about the user's upcoming movies and series, as well as a custom
   * message.
   *
  ...
  public void sendWeeklySummaryEmail(
      String userId, String title, String content, int upcomingMovies, int upcomingSeries) {
    Optional<User> userOpt = userRepository.findById(userId);
    if (userOpt.isEmpty()) return;

    User user = userOpt.get();
    String htmlContent =
        emailTemplateService.createWeeklySummaryTemplate(
            user.getUsername(), upcomingMovies, upcomingSeries, content);
    sendHtmlNotificationEmail(user.getEmail(), title, htmlContent);
  }

  /**
   * Sends a milestone email asynchronously to a user. The email contains details about the achieved
   * milestone, such as its type and count.
   *
   * @param userId the unique identifier of the user to whom the email will be sent
  ...
  public void sendMilestoneEmail(String userId, String milestoneType, int count) {
    ...
    User user = userOpt.get();
    String htmlContent =
        emailTemplateService.createMilestoneTemplate(user.getUsername(), milestoneType, count);
   ...
   * about a recommended item such as its title, type, poster image, reason for recommendation, and
   * score.
   *
  ...
  public void sendRecommendationEmail(
    ...
    User user = userOpt.get();
    String htmlContent =
        emailTemplateService.createRecommendationTemplate(
            user.getUsername(), title, message, itemId, itemType, posterUrl, reason, score);
    sendHtmlNotificationEmail(user.getEmail(), title, htmlContent);
  }
}
