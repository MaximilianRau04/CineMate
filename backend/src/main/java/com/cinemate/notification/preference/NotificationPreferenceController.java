...
public class NotificationPreferenceController {
  ...
  public ResponseEntity<List<NotificationPreference>> getUserNotificationPreferences(
  ...
  public ResponseEntity<List<NotificationPreference>> updateUserNotificationPreferences(
      @PathVariable String userId, @RequestBody List<NotificationPreference> preferences) {
  ...
  public ResponseEntity<Map<String, Boolean>> updateGlobalNotificationSettings(
      @PathVariable String userId, @RequestBody Map<String, Boolean> settings) {
  ...
  public ResponseEntity<Map<String, Boolean>> getGlobalNotificationSettings(
      @PathVariable String userId) {
    Optional<User> userOpt = userRepository.findById(userId);
    if (userOpt.isEmpty()) {
      return ResponseEntity.notFound().build();
  ...
  private boolean isEmailEnabledByDefault(NotificationType type) {
    return switch (type) {
      case MOVIE_WATCHLIST_RELEASED,
              ...
              ADMIN_NOTIFICATION ->
          true;
      case WATCHLIST_ITEM_REVIEWED, FAVORITE_ITEM_REVIEWED, SYSTEM_ANNOUNCEMENT -> false;
      default -> true;
    };
  }
}
