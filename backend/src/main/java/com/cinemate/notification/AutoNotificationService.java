...
public class AutoNotificationService {
    ...
    users.stream()
        ...
        .forEach(
            user -> {
              ...
              String message =
                  String.format(
                      "Der Film '%s' aus deiner Watchlist ist jetzt verfügbar!", movie.getTitle());
  ...
  public void notifyDailyReleases(List<Movie> releasingMovies, List<Series> releasingSeries) {
    ...
    for (User user : users) {
      ...
      StringBuilder messageBuilder =
          new StringBuilder("Heute erscheinen Inhalte aus deiner Watchlist:\n\n");
    ...
    users.stream()
        ...
        .forEach(
            user -> {
              ...
              String message =
                  String.format(
                      "Staffel %d von '%s' ist jetzt verfügbar!",
    ...
    users.stream()
        ...
        .forEach(
            user -> {
              ...
              String message =
                  String.format(
                      "Episode %d von '%s' (Staffel %d) ist jetzt verfügbar!",
    ...
    users.stream()
        ...
        .forEach(
            user -> {
              ...
              String message =
                  String.format(
                      "Der Status von '%s' hat sich von '%s' zu '%s' geändert.",
  ...
  public void notifyWatchlistItemReviewed(Review review, String itemTitle, String itemType) {
    ...
    users.stream()
        ...
        .filter(
            user ->
                !user.getId()
                    .equals(review.getUserId())) // Nicht den Autor der Bewertung benachrichtigen
        .forEach(
            user -> {
              String title = "⭐ Neue Bewertung verfügbar";
              String message =
                  String.format(
                      "'%s' aus deiner Watchlist wurde mit %.1f Sternen bewertet.",
    ...
    users.stream()
        ...
        .forEach(
            user -> {
              ...
              String message =
                  String.format(
                      "'%s' aus deinen Favoriten wurde mit %.1f Sternen bewertet.",
                      itemTitle, review.getRating());
  ...
  public void notifyMilestoneReached(String userId, String milestoneType, int count) {
    ...
    switch (milestoneType) {
      ...
      default:
        message =
            String.format("Du hast einen Meilenstein erreicht: %s (%d)", milestoneType, count);
    }
  ...
  public void notifyUpcomingReleases(String userId) {
    ...
    List<Series> upcomingSeries =
        user.getSeriesWatchlist().stream()
            .filter(
                series ->
                    series.getReleaseDate() != null
    ...
    if (!upcomingMovies.isEmpty() || !upcomingSeries.isEmpty()) {
      ...
      if (!upcomingMovies.isEmpty()) {
        ...
        message.append(
            upcomingMovies.stream().map(Movie::getTitle).collect(Collectors.joining(", ")));
        message.append("\n");
      }
      ...
      if (!upcomingSeries.isEmpty()) {
        ...
        message.append(
            upcomingSeries.stream().map(Series::getTitle).collect(Collectors.joining(", ")));
      }
  ...
  public void notifyForumPostCreated(ForumPost forumPost) {
    ...
    users.stream()
        .filter(
            user -> !user.getId().equals(forumPost.getAuthor().getId())) // Don't notify the author
        .forEach(
            user -> {
        ...
        .filter(
            subscription ->
                !subscription
                    .getUser()
                    .getId()
                    .equals(forumReply.getAuthor().getId())) // Don't notify the reply author
        .forEach(
            subscription -> {
              String title = "💬 Neue Antwort auf abonnierten Beitrag";
              String message =
                  String.format(
                      "'%s' hat auf den Beitrag '%s' geantwortet",
