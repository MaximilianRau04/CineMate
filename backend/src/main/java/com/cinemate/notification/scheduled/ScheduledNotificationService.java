...
public class ScheduledNotificationService {
  ...
  /** sends weekly notifications about upcoming releases executed every Monday at 10am */
    ...
    users.forEach(
        user -> {
          try {
            ...
            log.error(
                "Error sending weekly notification to user "
                    + user.getId()
                    + ": "
                    + e.getMessage());
          }
  ...
  /** sends personalized recommendations to all users executed every Wednesday at 2pm */
  ...
  /** sends personalized recommendation summaries to all users executed every Friday at 6pm */
  ...
  /** checks daily for new releases (release date = today) executed every day at 9am */
  ...
  public void checkDailyReleases() {
    ...
    try {
      ...
      List<Movie> releasingMoviesToday =
      ...
      List<Series> releasingSeriesToday =
      ...
      log.info(
          "Found "
              + releasingMoviesToday.size()
              + " movies and "
              + releasingSeriesToday.size()
              + " series releasing today");
