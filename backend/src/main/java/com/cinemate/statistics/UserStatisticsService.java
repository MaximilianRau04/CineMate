...
public class UserStatisticsService {
   ...
   * This includes metrics such as total hours watched, top genres, favorite actors, and recent
   * activity.
   *
   * @param userId the unique identifier of the user whose statistics are to be calculated
   * @param period the time period for which statistics should be calculated (e.g., weekly, monthly,
   *     yearly)
   * @return an instance of {@code UserStatisticsDTO} containing the calculated statistics for the
   *     specified user and period
   ...
   * @return a list of FriendStatisticsDTO objects containing statistical information about each
   *     friend
   */
          ...
          .map(
              friendship -> {
                ...
                try {
                  ...
                  log.error(
                      "Error building FriendStatisticsDTO for friend: "
                          + friend.getUsername()
                          + " - "
                          + e.getMessage());
                  return null;
   ...
   *
   * @param period the period for which the start date is to be determined. Valid values are
   *     "month", "year", and "all".
  ...
  /**
   * Calculates the total number of hours of movies and series watched by the user starting from the
   * specified date.
   *
        ...
        .mapToInt(
            series -> {
              if (series.getSeasons() != null && !series.getSeasons().isEmpty()) {
                return series.getSeasons().stream()
                    .mapToInt(
                        season -> season.getEpisodes() != null ? season.getEpisodes().size() : 0)
                    .sum();
              }
              return 0;
            })
  ...
  /**
   * Calculates the average rating of reviews for a given user. If a start date is provided, only
   * reviews created after the specified start date are considered.
   *
  ...
  private Double calculateAverageRating(String userId, LocalDateTime startDate) {
    List<Review> reviews =
        startDate != null
            ? reviewRepository.findByUserIdAndDateAfter(
                userId, Date.from(startDate.atZone(ZoneId.systemDefault()).toInstant()))
  ...
  /**
   * Calculates the list of favorite actors based on the movies and series watched by the user
   ...
   * @return a list of ActorStatisticsDTO objects representing the top favorite actors and their
   *     respective counts, sorted in descending order
   */
  private List<ActorStatisticsDTO> calculateFavoriteActors(User user, LocalDateTime startDate) {
    ...
    return actorCounts.entrySet().stream()
        .map(
            entry ->
                ActorStatisticsDTO.builder().name(entry.getKey()).count(entry.getValue()).build())
        .sorted((a, b) -> b.getCount().compareTo(a.getCount()))
        .limit(10)
        .collect(Collectors.toList());
  }
  ...
  /**
   * Calculates the list of favorite directors for the given user based on the movies and series
   * they have watched. Only considers data created prior to the specified start date. The result is
   * a sorted list of directors in descending order of their occurrences.
   *
   ...
   */
  private List<DirectorStatisticsDTO> calculateFavoriteDirectors(
  ...
  /**
   * Calculates the monthly activity data for a user over a specified period of one year. The method
   * generates a list of monthly activity summaries including details like viewing hours, movie
   * count, and series count for each month.
   *
   ...
   * @return a list of MonthlyActivityDTO objects representing the activity data for each month in
   *     the defined period
   */
    ...
    userReviews.forEach(
        review -> {
          LocalDateTime reviewDate =
              LocalDateTime.ofInstant(review.getDate().toInstant(), ZoneId.systemDefault());
  ...
  /**
   * Calculates and retrieves a list of yearly activity data for the specified user. The activity
   * includes details like the number of hours spent, movies watched, and series watched over the
   * past three years.
   *
  ...
  private List<RecentActivityDTO> getRecentActivity(String userId) {
    ...
    // recent reviews
    List<Review> recentReviews = reviewRepository.findTop10ByUserIdOrderByDateDesc(userId);
                ...
                RecentActivityDTO.builder()
                    ...
                    .date(
                        LocalDateTime.ofInstant(
                            review.getDate().toInstant(), ZoneId.systemDefault()))
                    .build());
          } else {
  ...
  /**
   * Analyzes the user's watching behavior based on historical viewing data and calculates patterns,
   * such as the most active day, most active hour, average session length, and preferred genres by
   * time of day.
   *
   * @param userId the unique identifier of the user whose watching patterns are to be calculated
   * @return a WatchingPatternsDTO object containing the calculated watching patterns, including the
   *     most active day, most active hour, average session length, and a map of preferred genres
  ...
  private WatchingPatternsDTO calculateWatchingPatterns(String userId) {
    ...
    if (!preferredGenreByTime.containsKey("afternoon"))
      preferredGenreByTime.put("afternoon", "Drama");
